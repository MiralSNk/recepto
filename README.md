...
Три готовых, задокументированных до последней команды способа:
...
- **[`PROVISION.md`](./PROVISION.md)** — полностью автоматически через GitHub Actions: один запуск workflow превращает пустой сервер в готовый сайт с HTTPS, без единой ручной команды на самом сервере. По сути автоматизирует один из двух способов выше, на выбор при запуске.

Первые два — вручную по шагам, третий — вообще без ручного захода на сервер. Все три пути покрывают одно и то же: миграции БД, создание первого администратора без ручных SQL-вставок пароля, первый вход и заполнение реальных данных.

После первоначальной настройки любым способом — **[`CI_CD.md`](./CI_CD.md)** автоматизирует дальнейшие обновления: пуш в `main` сам прогоняет тесты и, если они прошли, обновляет код на сервере по SSH (GitHub Actions). PROVISION.md уже настраивает это как часть первого запуска.
