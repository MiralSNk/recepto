...
После первоначальной настройки — **[`CI_CD.md`](./CI_CD.md)** автоматизирует дальнейшие обновления: пуш в `main` сам прогоняет тесты и, если они прошли, обновляет код на сервере по SSH (GitHub Actions). Надстройка над уже настроенным сервером, а не третий отдельный способ развернуть с нуля.
