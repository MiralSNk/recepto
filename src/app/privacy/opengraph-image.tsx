...
import { OG_SIZE, getOgColors, getHotelInitial, isAbsoluteImageUrl, loadFont, getSiteSettings } from '@/lib/index.server';
...
export default async function Image() {
  ...
  const ogTitle = settings.og_privacy_title || 'Политика конфиденциальности — Название вашего отеля';
  const ogDescription = settings.og_privacy_description || 'Как мы обрабатываем персональные данные.';
  const ogImage = isAbsoluteImageUrl(settings.og_privacy_image) ? settings.og_privacy_image : '';
      ...
      >
        ...
        >
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: `2px solid ${OG_COLORS.gold}`, paddingTop: '24px' }}>
            ...
            <span style={{ fontSize: '18px', opacity: 0.9 }}>Конфиденциальность</span>
          </div>
        </div>
      </div>
    ),
