...
export default async function Image() {
  ...
  // Логотипы: сначала кастомные из БД, затем стандартные SVG из src/assets/icons
  const titleLogoSrc =
  ...
  const ogTitle = settings.og_home_title || 'Название вашего отеля — уютный отель в центре города';
  const ogDescription = settings.og_home_description || 'Комфортные номера и внимательный сервис';
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {titleLogoSrc && (
                <img src={titleLogoSrc} style={{ height: '36px', objectFit: 'contain' }} />
              )}
              {subtitleLogoSrc && (
                <img src={subtitleLogoSrc} style={{ height: '16px', objectFit: 'contain', opacity: 0.9 }} />
              )}
              {!titleLogoSrc && (
                <span style={{ fontSize: '28px', letterSpacing: '0.04em' }}>
                  {hotelName}
                </span>
              )}
              {!subtitleLogoSrc && (
                <span style={{ fontSize: '14px', textTransform: 'uppercase', letterSpacing: '0.1em', opacity: 0.9 }}>
                  {hotelAddress}
                </span>
              )}
            </div>
          ...
          >
            ...
            <div style={{ fontSize: '24px', maxWidth: '700px', opacity: 0.95 }}>
              {ogDescription}
            </div>
          </div>
          ...
          <div
            style={{
              ...
              justifyContent: 'space-between',
              alignItems: 'center',
              borderTop: `2px solid ${OG_COLORS.gold}`,
              paddingTop: '24px',
            }}
          >
